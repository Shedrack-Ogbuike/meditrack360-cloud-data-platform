...
# Extract
csv_files = [f"C:\\Users\\user\\Desktop\\MediTrack360\\services\\extract\\lab_results\\lab_results_2025-11-{i:02d}.csv" for i in range(1, 11)]
combined_df = pd.concat([pd.read_csv(f).assign(source_file=os.path.basename(f)) for f in csv_files if os.path.exists(f)])
