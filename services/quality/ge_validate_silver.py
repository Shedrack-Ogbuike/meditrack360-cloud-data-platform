...
def validate_silver_table(df, table_name):
    ...
    if table_name == "patients":
        ...
        duplicate_patients = (
            df.groupBy("patient_id").count().filter(F.col("count") > 1).count()
        )
        if duplicate_patients > 0:
            validation_results["issues"].append(
                f"{duplicate_patients} duplicate patient_ids"
            )
            validation_results["passed"] = False
        ...
        if "date_of_birth" in df.columns:
            ...
            if future_births > 0:
                validation_results["issues"].append(
                    f"{future_births} future birth dates"
                )
                validation_results["passed"] = False
        ...
        if "admission_date" in df.columns:
            future_admissions = df.filter(
                F.col("admission_date") > F.current_date()
            ).count()
            if future_admissions > 0:
                validation_results["warnings"] = [
                    f"{future_admissions} future admission dates (scheduled?)"
                ]
        ...
        if "sample_datetime" in df.columns:
            future_tests = df.filter(
                F.col("sample_datetime") > F.current_timestamp()
            ).count()
            if future_tests > 0:
...
def save_validation_results(spark, validation_results, table_name, execution_date):
    ...
    validation_path = (
        f"s3a://{bucket}/silver/validation/{execution_date}/{table_name}_validation"
    )
...
def main():
    ...
    save_validation_results(
        spark, pharmacy_validation, "pharmacy_inventory", execution_date
    )
    ...
    if all_passed:
        ...
        failed_tables = [
            v["table"] for v in all_validations if not v.get("passed", True)
        ]
