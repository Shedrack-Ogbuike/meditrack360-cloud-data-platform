    ...
    try:
        ...
        if table_name.startswith("dim_"):
            ...
            if "patient_key" in df.columns or "ward_key" in df.columns:
                ...
                if null_count > 0:
                    validation_results["issues"].append(
                        f"{null_count} null values in dimension key"
                    )
            ...
            if "patient_key" in df.columns:
                duplicate_count = (
                    df.groupBy("patient_key").count().filter(col("count") > 1).count()
                )
                if duplicate_count > 0:
                    validation_results["issues"].append(
                        f"{duplicate_count} duplicate patient keys"
                    )
                    validation_results["passed"] = False
            ...
            if table_name == "dim_patient" and "patient_key" in df.columns:
                ...
                if total_count != unique_keys:
                    validation_results["issues"].append(
                        f"Dimension keys not unique: {total_count} rows, {unique_keys} unique keys"
                    )
                    validation_results["passed"] = False
        ...
        # Fact table validations
            ...
            if "patient_id" in df.columns:
                ...
                if null_patients > 0:
                    validation_results["issues"].append(
                        f"{null_patients} null patient_ids in fact table"
                    )
                    validation_results["passed"] = False
            ...
            if table_name == "fact_admissions" and "admission_id" in df.columns:
                ...
                if null_admissions > 0:
                    validation_results["issues"].append(
                        f"{null_admissions} null admission_ids"
                    )
                    validation_results["passed"] = False
        ...
        # Summary table validations
            ...
            if "total_admissions" in df.columns:
                ...
                if negative_admissions > 0:
                    validation_results["issues"].append(
                        f"{negative_admissions} negative admission counts"
                    )
                    validation_results["passed"] = False
            ...
            if "age" in df.columns:
                ...
                if invalid_age > 0:
                    validation_results["issues"].append(
                        f"{invalid_age} invalid age values"
                    )
                    validation_results["passed"] = False
            ...
            if "daily_admissions" in df.columns:
                ...
                if negative_daily > 0:
                    validation_results["issues"].append(
                        f"{negative_daily} negative daily admissions"
                    )
                    validation_results["passed"] = False
            ...
            if "unique_patients" in df.columns and "daily_admissions" in df.columns:
                ...
                illogical = df.filter(
                    col("unique_patients") > col("daily_admissions")
                ).count()
                if illogical > 0:
                    validation_results["issues"].append(
                        f"{illogical} rows where unique_patients > daily_admissions"
                    )
                    validation_results["passed"] = False
...
def save_gold_validation_results(spark, validation_results, table_name, execution_date):
    ...
    validation_path = (
        f"s3a://{bucket}/gold/validation/{execution_date}/{table_name}_validation"
    )
...
def check_referential_integrity(spark, bucket, execution_date):
    ...
    try:
        ...
        valid_patient_ids = {
            row["patient_id"]
            for row in dim_patient.select("patient_id").distinct().collect()
        }
        valid_ward_ids = {
            row["ward_id"] for row in dim_ward.select("ward_id").distinct().collect()
        }
        ...
        if "patient_id" in fact_admissions.columns:
            missing_patients = fact_admissions.filter(
                ~col("patient_id").isin(list(valid_patient_ids))
            ).count()
            ...
            if missing_patients > 0:
                ...
                logger.warning(
                    f"Referential integrity: {missing_patients} patient_ids in fact_admissions not found in dim_patient"
                )

        # Wards in fact_admissions not in dim_ward
        if "ward_id" in fact_admissions.columns:
            missing_wards = fact_admissions.filter(
                ~col("ward_id").isin(list(valid_ward_ids))
            ).count()
            ...
            if missing_wards > 0:
                integrity_results["overall_status"] = "FAIL"
                logger.warning(
                    f"Referential integrity: {missing_wards} ward_ids in fact_admissions not found in dim_ward"
                )
    ...
    integrity_path = (
        f"s3a://{bucket}/gold/validation/{execution_date}/referential_integrity"
    )
...
def transform_silver_to_gold():
    ...
    try:
        ...
        save_gold_validation_results(
            spark, patient_validation, "dim_patient", execution_date
        )
        ...
        save_gold_validation_results(
            spark, admissions_validation, "fact_admissions", execution_date
        )
        ...
        if (
            patient_validation["passed"]
            and admissions_validation["passed"]
            and integrity_results["overall_status"] == "PASS"
        ):
