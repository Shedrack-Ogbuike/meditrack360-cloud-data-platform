...
class LabDataValidator:
    ...
    def validate_lab_results(
        self, df: pd.DataFrame, file_stats: Optional[List[Dict]] = None
    ) -> LabValidationResult:
        """
        ...
        result.passed = self._determine_validation_pass(
            result.critical_issues, result.row_count
        )
    ...
    def _validate_patient_ids(self, df: pd.DataFrame) -> Tuple[List[str], List[str]]:
        ...
        if invalid_format.sum() > 0:
            warnings.append(
                f"Found {invalid_format.sum()} tests with unusual patient ID format"
            )
        ...
        if "patient_id" in df.columns:
            ...
            if unusual_length > 0:
                warnings.append(
                    f"Found {unusual_length} tests with unusual patient ID length"
                )
    ...
    def _validate_test_types(self, df: pd.DataFrame) -> Tuple[List[str], List[str]]:
        ...
        if len(rare_tests) > 0:
            warnings.append(
                f"Found {len(rare_tests)} very rare test types (potential typos)"
            )
        ...
        result.test_type_stats["test_type_distribution"] = std_test_type_counts.head(
            20
        ).to_dict()
    ...
    def _validate_test_dates(self, df: pd.DataFrame) -> Tuple[List[str], List[str]]:
        ...
        try:
            ...
            if len(invalid_dates) > 0:
                issues.append(
                    f"Found {len(invalid_dates)} tests with invalid date format"
                )

            # 2. Future test dates
            now = pd.Timestamp.now()
            future_cutoff = now + timedelta(
                days=self.config["future_date_tolerance_days"]
            )
            ...
            if len(far_future) > 0:
                issues.append(
                    f"Found {len(far_future)} tests with dates >{self.config['future_date_tolerance_days']} days in future"
                )

            if len(near_future) > 0:
                warnings.append(
                    f"Found {len(near_future)} tests with dates up to {self.config['future_date_tolerance_days']} days in future"
                )
            ...
            if len(old_tests) > 0:
                warnings.append(
                    f"Found {len(old_tests)} tests older than {self.config['max_age_years']} years"
                )

            # 4. Calculate date statistics
            if "_test_dt" in df.columns and len(df[df["_test_dt"].notna()]) > 0:
                result.test_type_stats["latest_test_date"] = (
                    df["_test_dt"].max().isoformat()
                )
                result.test_type_stats["earliest_test_date"] = (
                    df["_test_dt"].min().isoformat()
                )
    ...
    def _validate_result_values(self, df: pd.DataFrame) -> Tuple[List[str], List[str]]:
        ...
        if len(non_numeric) > 0:
            ...
            warnings.append(
                f"Found {len(non_numeric)} non-numeric results for test types: {list(non_numeric_types)}"
            )
        ...
        if "_test_type_std" in df.columns:
            ...
            positive_tests = numeric_tests[
                numeric_tests["_test_type_std"].str.contains(
                    "glucose|cholesterol|hemoglobin|wbc|rbc|platelets"
                )
            ]

            negative_values = positive_tests[
                positive_tests["_result_numeric"] < self.config["min_result_value"]
            ]
            if len(negative_values) > 0:
                issues.append(
                    f"Found {len(negative_values)} tests with negative values (should be positive)"
                )
    ...
    def _validate_temporal_consistency(self, df: pd.DataFrame) -> List[str]:
        ...
        if "_test_dt" in df.columns and "patient_id" in df.columns:
            ...
            if len(multiple_same_day) > 0:
                warnings.append(
                    f"Found {len(multiple_same_day)} patient-date combinations with multiple tests"
                )
    ...
    def _validate_test_frequency(self, df: pd.DataFrame) -> List[str]:
        ...
        if "_test_dt" in df.columns and "patient_id" in df.columns:
            ...
            patient_tests["_days_between"] = (
                patient_tests.groupby("patient_id")["_test_dt"].diff().dt.days
            )

            # Check for unusually frequent testing
            too_frequent = patient_tests[
                patient_tests["_days_between"] < 1
                ...
                if len(test_matches) > 0:
                    ...
                    if total_abnormal > 0:
                        ...
                        if abnormal_pct > 0.5:  # More than 50% abnormal
                            issues.append(
                                f"{test_name}: {total_abnormal} abnormal results ({abnormal_pct:.1%})"
                            )
                        else:
                            warnings.append(
                                f"{test_name}: {total_abnormal} abnormal results ({abnormal_pct:.1%})"
                    ...
                    if len(test_matches) > 10:  # Need enough data
                        ...
                        if std > 0:
                            outliers = test_values[
                                abs(test_values - mean)
                                > self.config["outlier_sd_threshold"] * std
                            ]

                            if len(outliers) > 0:
                                warnings.append(
                                    f"{test_name}: {len(outliers)} extreme outliers"
                                )
    ...
    def _calculate_summary(
        self, df: pd.DataFrame, result: LabValidationResult
    ) -> Dict[str, Any]:
        ...
        summary["unique_test_types"] = (
            df["test_type"].nunique() if "test_type" in df.columns else 0
        )
        ...
        if (
            "_result_numeric" in df.columns
            and len(df[df["_result_numeric"].notna()]) > 0
        ):
