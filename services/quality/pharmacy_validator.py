...
class PharmacyDataValidator:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.drug_categories = [
            ...
            'vaccine', 'insulin', 'chemotherapy', 'anesthetic'
        ]
    ...
    def _validate_drug_names(self, df: pd.DataFrame) -> List[str]:
        ...
        if len(short_names) > 0:
            warnings.append(f"Found {len(short_names)} very short drug names (< 2 chars)")
        ...
        if len(numeric_names) > 0:
            warnings.append(f"Found {len(numeric_names)} drug names that are only numbers")
    ...
    def _validate_quantities(self, df: pd.DataFrame) -> tuple:
        ...
        if len(high_qty) > 0:
            warnings.append(f"Found {len(high_qty)} drugs with unusually high quantity (> {self.config['high_quantity_threshold']})")
        
        # 3. Maximum quantity check
        max_qty = df[df['_quantity_numeric'] > self.config['max_quantity']]
        if len(max_qty) > 0:
            issues.append(f"Found {len(max_qty)} drugs exceeding maximum quantity ({self.config['max_quantity']})")
        
        # 4. Zero quantity warning
        zero_qty = df[df['_quantity_numeric'] == 0]
        if len(zero_qty) > 0:
            warnings.append(f"Found {len(zero_qty)} drugs with zero quantity (may be out of stock)")
        
        # 5. Non-numeric quantities
        non_numeric = df[df['_quantity_numeric'].isna()]
        if len(non_numeric) > 0:
            issues.append(f"Found {len(non_numeric)} drugs with non-numeric quantity values")
    ...
    def _validate_expiry_dates(self, df: pd.DataFrame) -> tuple:
        ...
        try:
            ...
            if len(invalid_dates) > 0:
                issues.append(f"Found {len(invalid_dates)} drugs with invalid expiry date format")
            ...
            if len(expired) > self.config['expired_drug_tolerance']:
                issues.append(f"Found {len(expired)} expired drugs (tolerance: {self.config['expired_drug_tolerance']})")
            
            # 3. Drugs expiring soon (within buffer period)
            buffer_date = now + timedelta(days=self.config['expiry_buffer_days'])
            expiring_soon = df[
                (df['_expiry_dt'] >= now) & 
                (df['_expiry_dt'] <= buffer_date)
            ]
            
            if len(expiring_soon) > 0:
                warnings.append(f"Found {len(expiring_soon)} drugs expiring within {self.config['expiry_buffer_days']} days")
            ...
            if len(far_future) > 0:
                warnings.append(f"Found {len(far_future)} drugs with expiry dates >5 years in future")
            ...
            if len(ancient_drugs) > 0:
                issues.append(f"Found {len(ancient_drugs)} drugs with expiry dates before 2000")
    ...
    def _validate_drug_categories(self, df: pd.DataFrame) -> List[str]:
        ...
        if 'category' in df.columns:
            ...
            if unknown_cats:
                warnings.append(f"Found unknown drug categories: {list(unknown_cats)[:5]}")
            
            # Check for empty categories
            empty_cats = df[df['category'].isnull() | (df['category'].astype(str).str.strip() == '')]
            if len(empty_cats) > 0:
    ...
    def _determine_validation_pass(self, issues: List[str], 
                                 row_count: int, 
                                 expired_drugs: int) -> bool:
