...
class PostgresDataValidator:
    ...
    def _validate_primary_key(self, df: pd.DataFrame, pk_column: str) -> List[str]:
        ...
        if duplicate_count > 0:
            issues.append(
                f"{duplicate_count} duplicate values in primary key {pk_column}"
            )
    ...
    def _validate_patient_id_format(self, patient_ids: pd.Series) -> List[str]:
        ...
        invalid_ids = patient_ids[
            ~patient_ids.astype(str).str.match(r"^[A-Za-z0-9\-_]+$")
        ]
    ...
    def _validate_patient_dates(self, df: pd.DataFrame) -> tuple:
        ...
        try:
            ...
            if len(future_births) > 0:
                ...
                reasonable_future = future_births[
                    future_births["_dob_temp"] <= max_future
                ]
                unreasonable_future = future_births[
                    future_births["_dob_temp"] > max_future
                ]

                if len(unreasonable_future) > 0:
                    issues.append(
                        f"{len(unreasonable_future)} patients with birth dates >{self.config['max_future_years']} years in future"
                    )

                if len(reasonable_future) > 0:
                    warnings.append(
                        f"{len(reasonable_future)} patients with future birth dates (within {self.config['max_future_years']} years)"
                    )
    ...
    def _validate_patient_ages(self, df: pd.DataFrame) -> List[str]:
        ...
        try:
            ...
            if len(too_young) > 0:
                warnings.append(
                    f"{len(too_young)} patients with age < {self.config['min_patient_age']}"
                )

            if len(too_old) > 0:
                warnings.append(
                    f"{len(too_old)} patients with age > {self.config['max_patient_age']}"
                )
    ...
    def _validate_names(self, df: pd.DataFrame, column: str) -> List[str]:
        ...
        unusual_names = df[
            df[column].astype(str).str.contains(r"[0-9@#$%^&*]", na=False)
        ]

        if len(unusual_names) > 0:
            warnings.append(
                f"{len(unusual_names)} {column} values with unusual characters"
            )
    ...
    def _validate_ward_capacity(self, df: pd.DataFrame) -> List[str]:
        ...
        if len(unrealistic_capacity) > 0:
            issues.append(
                f"{len(unrealistic_capacity)} wards with unrealistic capacity"
            )
    ...
    def _create_result(
        ...
        if len(df) > 0:
            ...
            if issue_ratio > self.config["critical_issue_threshold"]:
                issues.append(
                    f"Issue ratio ({issue_ratio:.1%}) exceeds threshold ({self.config['critical_issue_threshold']:.1%})"
                )
                passed = False
